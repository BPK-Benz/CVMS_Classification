...
for temp, name_type in zip([temp_O],[name_type_O]): ################# Change #################
    ...
    for tlabel in ['CVMS_Ton','CVMS_Jam', 'CVMS_D']:

        label = temp.reset_index(drop=True).loc[:, tlabel]


        name_type = simplify_name(tlabel)
        noclass = simplify(tlabel)
        name_feat = os.getcwd()+'/Output_Project/Prepare_Stage2/Data_'+name_type+'_'+noclass+'.csv'
        ...
        name_feat = os.getcwd()+'/Output_Project/FeatureImportance/Feature_'+name_type+'_'+noclass+'.csv'
        ...
        name_feat = os.getcwd()+'/Output_Project/Prepare_Stage2/DataImportance_'+name_type+'_'+noclass+'.csv'
        pd.concat([scale_df.loc[:,feat],label],axis=1).to_csv(name_feat)

        labels = sorted(list(set(label)))
        model_score=[]

        # Selected single model: KNeighborsClassifier, Support Vector Machine, RandomForest, LogistricRegression
        # Selected ensemble model: GradientBoostingClassifier
        algo = [
        ...
        for a,b in zip(algo,parameters):
            ...
            name_model = os.getcwd()+'/Output_Project/Model/'+name_type+'_'+noclass+'_'+a[1]+'.joblib'
            ...
            name_para = os.getcwd()+'/Output_Project/BestParameters/'+name_type+'_'+noclass+'_'+a[1]+'_bestPara.csv'
            ...
            name_img = os.getcwd()+'/Output_Project/ConfusionMatrix/'+name_type+'_'+noclass+'_'+a[1]+'.png'
            ...
            name = os.getcwd()+'/Output_Project/Classification_report/'+name_type+'_'+noclass+'_'+a[1]+'_class_report.csv'
        ...
        name_model = os.getcwd()+'/Output_Project/Model/'+name_type+'_'+noclass+'_'+'MLP.joblib'
        dump(clf, name_model) 

        print('Best_parameter:', clf.best_params_)

        name_para = os.getcwd()+'/Output_Project/BestParameters/'+name_type+'_'+noclass+'_MLP_bestPara.csv'
        ...
        name_img = os.getcwd()+'/Output_Project/ConfusionMatrix/'+name_type+'_'+noclass +'_MLP.png'
        confusion_matrix_graph(y_test, y_pred, labels, name_img)
        ...
        name = os.getcwd()+'/Output_Project/Classification_report/'+name_type+'_'+noclass+'_MLP_class_report.csv'
        pd.DataFrame(report).transpose().to_csv(name)
        print('-' * 100)
        ...
        final.to_csv(os.getcwd()+'/Output_Project/ModelAccuracy/'+ name_type + '_CVMS_'+ noclass + '.csv')
